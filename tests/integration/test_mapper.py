...
# %%
df.columns = ["variable_names", "cell_methods", "dimensions", "branded_label-spreadsheet", "branded_label-following_appendixF-from_last_year", "matching"]
df

# %%
df.to_csv(DATA_ROOT / Path("compare_branded_variables-last_year_definitions.csv"), index=False)
#df.to_csv(DATA_ROOT / Path("compare_branded_variables.csv"), index = False)

# %%
tas = df[df["variable_names"] == "diftrebo"]
tas

# %%
print(tas.to_markdown(index=False))

# %%
#df[df["branded_label-spreadsheet"] == df["branded_label-following_appendixF"]]
df[df["branded_label-spreadsheet"] == df["branded_label-following_appendixF-from_last_year"]]
